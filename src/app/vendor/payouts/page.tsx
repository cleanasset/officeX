...
export default function VendorInvoicesAndPayouts() {
  ...
  const handleExportInvoices = () => {
    const headers = ["Invoice No", "WO ID", "Period", "Gross Amount", "Platform Fee", "Net Payout", "Paid Date", "Transaction ID", "Status"];
    const rows = invoices.map(inv => [inv.no, inv.wo, inv.period, `"${inv.gross}"`, `"${inv.fee}"`, `"${inv.net}"`, inv.date, inv.txId, inv.status]);
    ...
    link.href = url;
    link.setAttribute("download", `OfficeX_Vendor_Invoice_Ledger_${new Date().toISOString().split("T")[0]}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);

    setToast("Vendor invoice ledger downloaded as CSV!");
    setTimeout(() => setToast(null), 3000);
  };
